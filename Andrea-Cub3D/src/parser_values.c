...
/*   Updated: 2024/03/29 10:05:02 by vbartos          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*value_parser(char *value, char **parsed_file)
{
	...
	{
		j = increment_if_space(parsed_file[i], 0);
		str_start = j;
 ...
 */
void	set_floor_clr(char **split_rgb, t_game *game)
{
	int	i;
	int	ptr;
 ...
 * @param game      A pointer to the game structure.
 */
void	set_ceiling_clr(char **split_rgb, t_game *game)
{
	int	i;
	int	ptr;
...
void	clr_parser(char *value, char **parsed_file, t_game *game)
{
	...
	check_for_two_commas(rgb, game);
	split_rgb = ft_split(rgb, ',');
	free(rgb);
	check_clr_format(split_rgb, game);
	assign_color(value, split_rgb, game);
	free_str_arr(split_rgb);
}
