...
struct CountingLess {
	...
	bool operator()(const L &l, const R &r) const {
		return PmergeMe::countedLess(l, r);
	}
};

static size_t	fordJohnsonBound(size_t n)
{
	size_t	sum = 0;
	...
	for (size_t k = 1; k <= n; ++k)
	{
		...
		sum += static_cast<size_t>(std::ceil(std::log(value) / std::log(2.0)));
	}
	return sum;
}
...
static int	makeSmallBig(std::vector<Pair> &pairs, C &container)
{
	...
	for (size_t i = 1; i < container.size(); i += 2)
	{
		...
		Pair	p;
		if (PmergeMe::countedLess(frst, scnd)) {
			p.small = frst; p.big = scnd;
		} else {
			p.small = scnd; p.big = frst;
		}
		pairs.push_back(p);
	}
						   ...
						   std::vector<size_t> &bigPos)
{
	...
	for (size_t oi = 0; oi < order.size(); ++oi)
	{
		...
		typename C::iterator pos =
			std::lower_bound(cont.begin(), bound, pairs[idx].small, cmp);
...
static double	FordJohnsonAlg(C &container)
{
	...
	{
		typename C::iterator pos =
			std::lower_bound(container.begin(), container.end(), oddStraggler, CountingLess());
...
void	PmergeMe::sort()
{
	...
	size_t	compVector = PmergeMe::nbr_of_comps;

	PmergeMe::nbr_of_comps = 0;
	double	timeDeque = FordJohnsonAlg(dSorted);
	size_t	compDeque = PmergeMe::nbr_of_comps;
	...
	size_t	expected = fordJohnsonBound(vSorted.size());
	std::cout << "\nComparisons with std::vector : " << compVector
			  << " | expected upper-bound F(n) = " << expected << "\n";
	std::cout << "Comparisons with std::deque  : " << compDeque
			  << " | expected upper-bound F(n) = " << expected << "\n";
}
