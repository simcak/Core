...
BUCKET = os.getenv("MINIO_COVERS_BUCKET", "covers")


def upload_cover(storage_key: str, data: bytes, mime_type: str) -> None:
    minio_client.put_object(BUCKET, storage_key, BytesIO(data), length=len(data), content_type=mime_type)


def delete_cover(storage_key: str) -> None:
    minio_client.remove_object(BUCKET, storage_key)


def get_cover_stream(storage_key: str):
